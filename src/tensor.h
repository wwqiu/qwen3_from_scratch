...
public:
    ...
    Tensor (std::vector<int64_t> shape, size_t elem_size) : shape_{shape} {
    ...
    Tensor (int n, int c, int h, int w, int elem_size) : shape_{n, c, h, w} {
        data_ = new uint8_t[n * c * h * w * elem_size];
    }

    // Add move constructor and move assignment operator for efficient resource management
    Tensor(Tensor&& other) noexcept : shape_(std::move(other.shape_)), data_(other.data_) {
